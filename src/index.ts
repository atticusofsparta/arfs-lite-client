...
class ArFSClient implements ArFSClientType {
  ...
  public async getOwnerForDriveId(driveId: EntityID): Promise<ArweaveAddress> {
    ...
    const newDriveId = async () => {
      ...
      if (!edges.length) {
        throw new Error(
          `Could not find a transaction with "Drive-Id": ${driveId.toString()}`,
        );
      }
  ...
  async getDriveIDForEntityId(
    ...
    const newEntityId = await (async () => {
      ...
      if (!edges.length) {
        throw new Error(`Entity with ${gqlTypeTag} ${entityId.toString()} not found!`);
      }
        ...
        tags: [
          {
            ...
            value: folderIDs.map((id) => id.toString())
          },
  ...
  async getAllFoldersOfPublicDrive({
    ...
    while (hasNextPage) {
      const gqlQuery = buildQuery({
        tags: [
          { name: "Drive-Id", value: `${driveId.toString()}` },
